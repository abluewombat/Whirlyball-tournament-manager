...
function reorderPlanToAvoidSameFirstLast(plan: DivisionDayPlan[], fallbackOrder: string[]) {
  ...
  if (allowedSoftSplitPlan(ordered, ordered[0].division)) return ordered;
  const replacementIndex = ordered.findIndex((item, index) => index > 0 && index < ordered.length - 1 && item.division !== ordered[0].division);
...
function allowedSoftSplitPlan(plan: DivisionDayPlan[], division: string) {
  const indexes = plan.flatMap((item, index) => (item.division === division ? [index] : []));
  return indexes.length === 2 && indexes[1] - indexes[0] === 2;
}

function splitLargeDivisionBlocks(plan: DivisionDayPlan[], courts: number) {
  if (plan.length <= 1) return plan;
  const primaryBlocks: DivisionDayPlan[] = [];
  ...
  const ordered = [...primaryBlocks];
  for (const block of secondBlocks) {
    const firstIndex = ordered.findIndex((item) => item.division === block.division);
    const insertIndex = firstIndex >= 0 ? firstIndex + 2 : ordered.length;
    if (
      insertIndex <= ordered.length &&
      ordered[insertIndex - 1]?.division !== block.division &&
      ordered[insertIndex]?.division !== block.division
    ) {
      ordered.splice(insertIndex, 0, block);
    } else {
      ordered.push(block);
    }
  }
...
function orderDivisionDayPlan(plan: DivisionDayPlan[], fallbackOrder: string[], courts: number) {
  const ordered = plan.sort((left, right) => fallbackOrder.indexOf(left.division) - fallbackOrder.indexOf(right.division));
  return reorderPlanToAvoidSameFirstLast(splitLargeDivisionBlocks(ordered, courts), fallbackOrder);
}
...
function buildDivisionDayPlans({
  ...
  for (const [dayIndex, day] of seedingDays.entries()) {
    ...
    plans.set(dayKey, orderDivisionDayPlan(plan, order, input.courts));
  }
...
function isOpenScheduleSlot(game: GeneratedGame) {
  return game.phase === "seeding" && game.division === openSlotDivision && game.team1Id === null && game.team2Id === null && game.refTeamId === null;
}

function addOpenSeedingSlotGames(
  games: GeneratedGame[],
  seedingDays: Date[],
  input: ScheduleInput,
  dayStartMinutes: number,
  ...
  const occupied = new Set(games.map((game) => `${game.startsAt}:${game.court}`));
  for (const day of seedingDays) {
    ...
    for (let row = 0; row < rowCapacity; row++) {
      ...
      for (let court = 1; court <= input.courts; court++) {
        const key = `${startsAt}:${court}`;
        if (occupied.has(key)) continue;
        games.push({
          phase: "seeding",
          division: openSlotDivision,
          court,
          startsAt,
          team1Id: null,
          team2Id: null,
...
function buildPreplayedSeedingMatchups(teams: TeamRow[]) {
  return manualPreplayedSeedingMatchups.flatMap((matchup) => {
    ...
    const team2 = findManualTeam(teams, matchup.b);
    return team1 && team2
      ? [{ a: team1, b: team2, count: matchup.count, removeAllGenerated: matchup.removeAllGenerated, requiredRemainingGenerated: matchup.requiredRemainingGenerated || 0 }]
      : [];
  });
}
...
function applyPreplayedSeedingAdjustments(queues: Map<string, Matchup[]>, targetGamesByTeam: Map<number, number>, preplayedMatchups: ReturnType<typeof buildPreplayedSeedingMatchups>) {
  for (const preplayed of preplayedMatchups) {
    ...
    for (const matchup of queue) {
      if (matchupKey(matchup.a, matchup.b) !== matchupKey(preplayed.a, preplayed.b)) continue;
      matchup.required = true;
      marked++;
      if (marked >= preplayed.requiredRemainingGenerated) break;
    }
  }
}

function protectedSeedingPairKeys(queues: Map<string, Matchup[]>) {
  const keys = new Set<string>();
  for (const queue of queues.values()) {
    for (const matchup of queue) {
      if (matchup.required) keys.add(matchupKey(matchup.a, matchup.b));
    }
  }
  return keys;
}
...
function equalizeSeedingGameCounts(
  games: GeneratedGame[],
  byDivision: Map<string, TeamRow[]>,
  targetGamesByTeam = new Map<number, number>(),
  protectedPairKeys = new Set<string>()
) {
  ...
  if (targetGamesByTeam.size) {
    while (true) {
      const index = games.findLastIndex((game) => {
        ...
        if (protectedPairKeys.has(matchupKeyFromIds(game.team1Id, game.team2Id))) return false;
  ...
  while (teamIds.length) {
    ...
    for (let index = games.length - 1; index >= 0; index--) {
      ...
      if (protectedPairKeys.has(matchupKeyFromIds(game.team1Id, game.team2Id))) continue;
...
function matchupBlockedByTarget(
  matchup: Matchup,
  targetGamesByTeam: Map<number, number>,
  targetGamesByDivision: Map<string, number>,
  teamGameCounts: Map<number, number>,
  ...
  if (!targetGamesByTeam.size) return false;
  const aAtTarget = teamAtTarget(matchup.a, targetGamesByTeam, teamGameCounts);
  const bAtTarget = teamAtTarget(matchup.b, targetGamesByTeam, teamGameCounts);
  if (!aAtTarget && !bAtTarget) return false;
  if (aAtTarget && bAtTarget) return true;
...
function scoreAnchorMatchup(matchup: Matchup, anchor: TeamRow, opponent: TeamRow, teamGameCounts: Map<number, number>, pairPlayCounts: Map<string, number>, targetGamesByTeam: Map<number, number>) {
  ...
  const requiredPriority = matchup.required ? -1_000_000 : 0;
  ...
  return requiredPriority + pairCount * 100_000 + opponentTargetPenalty + opponentCount * 100 + Math.abs(anchorCount - opponentCount) * 10 + matchup.round;
}
...
function takeTeamFirstMatchup(
  ...
  const requiredCandidates = candidates.filter((candidate) => candidate.result.matchup.required);
  const underTargetCandidates = candidates.filter((candidate) => !teamAtTarget(candidate.team, targetGamesByTeam, teamGameCounts));
  const pool = requiredCandidates.length ? requiredCandidates : underTargetCandidates.length ? underTargetCandidates : candidates;
...
function repairSeedingSlots({
  ...
  for (const slot of slots) {
    ...
    while (availableCourts.length) {
      ...
      divisionGameCounts.set(result.matchup.division, (divisionGameCounts.get(result.matchup.division) || 0) + 1);
...
export async function generateSchedule(input: ScheduleInput): Promise<{
  ...
  for (const [dayIndex, day] of seedingDays.entries()) {
    ...
    const dayPlans = (buildDivisionDayPlans({
      seedingDays: seedingDays.slice(dayIndex),
    ...
    let planIndex = 0;
    let attemptsAtRow = 0;
    while (row < rowCapacity && dayPlans.length) {
      if (planIndex >= dayPlans.length) planIndex = 0;
      const dayPlan = dayPlans[planIndex];

      if (dayPlan.placedGames >= dayPlan.targetGames || dayPlan.placedRows >= Math.max(1, dayPlan.rowCount)) {
        dayPlans.splice(planIndex, 1);
        attemptsAtRow = 0;
        continue;
      }

      const divisionDeficit = divisionSeedingDeficit(dayPlan.division, seedingByDivision, targetGamesByTeam, teamGameCounts, queues);
      if (divisionDeficit <= 0) {
        dayPlans.splice(planIndex, 1);
        attemptsAtRow = 0;
        continue;
      }
      ...
      if (reservedCourts.size >= input.courts) {
        row++;
        attemptsAtRow = 0;
        continue;
      }
      ...
      const availableSeedingCourts = Math.min(input.courts - reservedCourts.size, divisionDeficit, dayPlan.targetGames - dayPlan.placedGames);
      ...
      if (!rowMatchups.length) {
        if (dayPlan.placedRows > 0 || dayPlan.placedGames > 0) {
          dayPlans.splice(planIndex, 1);
          attemptsAtRow = 0;
          continue;
        }
        planIndex = (planIndex + 1) % dayPlans.length;
        attemptsAtRow++;
        if (attemptsAtRow >= dayPlans.length) {
          row++;
          attemptsAtRow = 0;
        }
        continue;
  ...
  addOpenSeedingSlotGames(games, seedingDays, input, start, earlyStart, end);
